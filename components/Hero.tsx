		...
		>
			...
			<div className='mx-auto grid max-w-6xl gap-10 md:grid-cols-[1.2fr_0.9fr] md:items-center'>
				<div className='space-y-5'>
					<p className='mono text-xs uppercase tracking-[0.24em] text-[var(--muted)]'>#home</p>
					<h1 className='text-3xl font-semibold leading-[1.2] text-[var(--text)] sm:text-4xl md:text-5xl'>
						Anmol is a <span className='text-[var(--accent)]'>web designer</span> and{' '}
					...
					<p className='max-w-2xl text-sm leading-relaxed text-[var(--muted)] sm:text-base'>
						I craft responsive experiences where technology meets creativity. Clean layouts, performant builds, and a
						dash of motion to keep things memorable.
					</p>
					...
					<div className='grid grid-cols-3 gap-3 sm:max-w-md'>
						{stats.map(item => (
							<div key={item.label} className='panel rounded-sm px-3 py-3 text-sm text-[var(--text)]'>
					...
					>
						...
						<div className='flex h-full items-center justify-center'>
							<div className='h-[90%] w-[90%] rounded-sm border border-[var(--border)] bg-gradient-to-br from-[var(--surface)] to-[var(--surface-alt)] relative'>
								<Image src='/Anmol.jpg' alt="Anmol Rahangdale Full Stack Developer" fill={true} />
							</div>
						</div>
					</motion.div>
				</div>
